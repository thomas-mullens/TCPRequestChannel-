...
int main (int argc, char* argv[]) {
	...
	//		  dispatching handle_process_loop thread for new channel4

	// Call the constructor to create server-side control channel
	string str = "";
	TCPRequestChannel* control_channel = new TCPRequestChannel(str, to_string(r));
	while (true) {
		// Accept a new connection socket
		int accepted = control_channel->accept_conn();
		// Create a new channel with the accepted socket (the socket is already established)
		TCPRequestChannel* new_chan = new TCPRequestChannel(accepted);
		thread newThread (handle_process_loop, new_chan);
		// dispatch a new thread to handle_process_loop
		newThread.detach();
		// Since the main thread is not bloacked, it can accept more connections (use detach instead of join)
	}
